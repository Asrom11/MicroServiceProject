using System.Text;
using ExampleCore.ZookeperDistributedLock.Interfaces;
...
public class ZooKeeperDistributedSemaphore: IDistrebutedSemafor
{
    ...
    public ZooKeeperDistributedSemaphore(ZooKeeper zooKeeper, string semaphorePath, int maxCount)
    {
    ...
    public async Task<LockHandler> TryAcquireAsync(TimeoutValue timeout, CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                var watcher = new WaitCompletionSourceWatcher(waitCompletionSource);
